...
def misafir_islem_yap(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Giriş durum kontrolü
            if islem_turu_adi_lower == 'giriş' and misafir.durum == 'AKTIF':
                messages.error(request, f"'{misafir.ad} {misafir.soyad}' zaten aktif durumda. Tekrar giriş işlemi yapılamaz.")
                return redirect('misafir_detay', pk=misafir.pk)

            # Çıkış durum kontrolü
            if islem_turu_adi_lower == 'çıkış' and misafir.durum == 'PASIF':
                messages.error(request, f"'{misafir.ad} {misafir.soyad}' zaten pasif durumda. Tekrar çıkış işlemi yapılamaz.")
                return redirect('misafir_detay', pk=misafir.pk)

            # Giriş tarihini hem işlem hem misafir kaydına yaz
            ...
            if islem.islem_turu.durum_degistirir_mi and islem.islem_turu.yeni_durum:
                ...
                if islem_turu_adi_lower == 'çıkış':
                    ...
                    if misafir.yatak_no:
                        ...
                        messages.warning(request, f"'{misafir.ad} {misafir.soyad}' için zaten atanmış bir yatak bulunamadı.")

                messages.info(request, f"'{misafir.ad} {misafir.soyad}' için '{islem.islem_turu.ad}' işlemi başarıyla yapıldı ve durum '{misafir.get_durum_display()}' olarak güncellendi.")
            else:
